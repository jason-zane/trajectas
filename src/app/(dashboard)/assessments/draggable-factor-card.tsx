...
import { Brain, Dna, FileQuestion, CheckCircle, Plus } from "lucide-react"
...
export function DraggableFactorCard({
  factor,
  isAdded,
  onToggle,
}: DraggableFactorCardProps) {
  ...
  return (
    <div
      ref={ref}
      className={`group/factor relative flex items-center gap-2.5 rounded-lg border px-3 py-2.5 transition-all duration-200 cursor-grab active:cursor-grabbing select-none ${
        isAdded
    ...
    >
      <div className="flex size-7 shrink-0 items-center justify-center rounded-md bg-competency-bg">
        <Brain className="size-3.5 text-competency-accent" />
      </div>
      ...
      {isAdded ? (
        ...
        <button
          ...
          className="shrink-0 rounded-md p-0.5 text-muted-foreground opacity-0 group-hover/factor:opacity-100 hover:text-primary hover:bg-primary/10 transition-all"
          aria-label={`Add ${factor.name}`}
        >
