...
interface LibraryDimensionsTableProps {
  rows: TaxonomyAssignmentRow[];
  partnerId: string;
  isPlatformAdmin: boolean;
}

export function LibraryDimensionsTable({
  rows: initialRows,
  partnerId,
  isPlatformAdmin,
}: LibraryDimensionsTableProps) {
  ...
  function handleToggle(entityId: string, currentlyAssigned: boolean) {
    ...
    startTransition(async () => {
      ...
      toast.success(newState ? "Dimension enabled" : "Dimension disabled");
    });
  }
  ...
  function handleBulkToggle(assigned: boolean) {
    ...
    startTransition(async () => {
      ...
      toast.success(
        `${ids.length} dimension${ids.length !== 1 ? "s" : ""} ${assigned ? "enabled" : "disabled"}`,
      );
      setSelected(new Set());
    });
  }
