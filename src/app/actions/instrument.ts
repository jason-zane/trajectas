...
export async function draftBlueprintWithAI(
  ...
  const build = await getBuild(db, blueprint.buildId)
  ...
  const draftRequest: BlueprintDraftInput = {
    ...
    targetItemCount: build?.targetItemsPerConstruct ?? 10,
...
export async function generateItemsForBlueprint(
    ...
    try {
      ...
      const prompt = buildCellGenerationPrompt({
        ...
        measureType: isMeasureType(blueprint.measureType)
          ...
          : 'competency_behavioural',
        facetLabel: cell.facetLabel,
        facetDefinition: cell.facetDefinition ?? undefined,
        audienceLevel: String(build?.audience?.level ?? 'mixed'),
      ...
      if (parsed.items.length === 0) {
        ...
        const deduplicated = dedupeAgainst(parsed.items, [...seenStems])
        const kept = deduplicated.kept.slice(0, deficit)
  ...
  const runStatus: 'success' | 'failure' =
    failedCells > 0 || totalGenerated < coverage.cells.reduce((sum, cell) => sum + cell.deficit, 0) ? 'failure' : 'success'
...
export async function runFairnessScreenForBuild(buildId: string): Promise<{
  ...
  const items = (await listCandidateItemsForBuild(db, buildId)).filter(item => item.status !== 'rejected')
  ...
  const audienceLevel = typeof build.config?.readingLevel === 'string' ? build.config.readingLevel :
  ...
  for (let i = 0; i < items.length; i += chunkSize) {
    ...
    try {
      ...
      const response = await openRouterProvider.complete({
        ...
        prompt,
        systemPrompt: `${fairnessSystemPrompt}\nReturn a JSON object {"items":[...]} with one complete result per supplied ID.`,
        temperature: 0.3,
        maxTokens: 2000,
        responseFormat: 'json',
      })
      ...
      const { results, warnings: parseWarnings } = parseFairnessResponse(
        response.content,
        new Set(chunk.map(item => item.id)),
      )
      allWarnings.push(...parseWarnings)

      screenedCount += results.length
      // Persist clean reviews too, replacing stale flags.
  ...
  await recordStageRun(db, {
    ...
    status: screenedCount === items.length ? 'success' : 'failure',
    startedAt,
    completedAt,
    progressPct: 100,
    detail: `Screened ${screenedCount}/${items.length} items; ${flaggedCount} flagged; ${overCeilingCount} over reading ceiling`,
    outputSnapshot: {
      itemsScreened: screenedCount,
...
export async function previewPublish(buildId: string): Promise<{
  ...
  if (build.config?.likert) {
    try {
      const status = await getLikertStatus(db, buildId)
      if (!status?.ready) blockers.push('The current form has not completed automatic Likert review. Resume creation from the build page.')
    } catch (error) {
      blockers.push(error instanceof Error ? error.message : 'Unable to verify the current Likert specification.')
    }
  }
...
export async function publishBuild(
  ...
  if (build.config?.likert) await assertLikertPublishable(db, buildId, parsed.data.responseFormatId)
  ...
  if (formatRow.type === 'likert' && ['sjt', 'forced_choice'].includes(build.measureType)) {
    throw new Error('This measurement type cannot be published as Likert items.')
  }
      ...
      if (!constructId && bp.draftConstructName) {
        ...
        const { data: constructData, error: constructError } = await db
          ...
          .insert({
            ...
            definition: bp.draftConstructDefinition ?? null,
            is_active: true,
          })
          .select('id')
          .single()
...
/** Start a saved, resumable Likert pipeline; subsequent steps run from the build page. */
export async function quickBuildInstrumentAction(buildId: string, options: Record<string, unknown>) {
  await requireAdminScope()
  return startLikert(createAdminClient(), buildId, options)
}
