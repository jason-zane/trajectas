...
export function outcomeReportHtml(
  ...
  </style></head><body class="${sections.comparison ? "" : "brief"}"><div class="brand"><b>trajectas<span style="color:#2d6a5a;font-size:21pt">.</span></b><span>BUSINESS OUTCOMES · ${e(payload.study.clientName)}</span></div><header><p class="eyebrow">${e(metric.label)} · ${e(payload.config.periodStart)} to ${e(payload.config.periodEnd)}</p><h1>${e(payload.draft.headline)}</h1><p class="question muted">${e(payload.study.question)}</p></header>
