...
import { predictorLabel } from "./analysis";
import {
  metricValue,
  groupComparisonText,
  scenarioValues,
...
export function outcomeReportHtml(
  ...
  const e = escapeReportHtml,
    ...
    sections = reportSections(payload.draft);
  ...
  const technical = payload.result.results
    .map((r) => {
      ...
      return `<section class="technical-section"><h2>${e(m.label)}</h2><p>${e(r.model.method || r.model.unavailable)}. ${r.model.n} complete people; ${r.model.parameters} parameters. Context: ${e(r.model.controls.join(", ") || "None selected")}.</p>${r.model.details?.kind === "linear" ? `<p>In-sample R²: ${r.model.details.r2?.toFixed(3) ?? "—"}; adjusted R²: ${r.model.details.adjustedR2?.toFixed(3) ?? "—"}; added R² beyond context: ${r.model.details.addedR2?.toFixed(3) ?? "—"}.</p>` : ""}<table><thead><tr><th>Capability</th><th>People</th><th>Pearson r</th><th>Adjusted coefficient<br/>(95% interval)</th><th>FDR q</th><th>Evidence</th></tr></thead><tbody>${r.findings.map((f) => `<tr><td>${e(capabilityLabel(f.predictorId))}</td><td>${f.n}</td><td>${f.correlation?.value.toFixed(3) ?? "—"}</td><td>${f.adjusted ? `${f.adjusted.value.toFixed(3)}<br/>[${f.adjusted.lower.toFixed(3)}, ${f.adjusted.upper.toFixed(3)}]` : "Unavailable"}</td><td>${f.adjusted?.q?.toPrecision(3) ?? "—"}</td><td>${e(f.status)}</td></tr>`).join("")}</tbody></table>${r.findings.map((f) => `<p class="caption">${e(groupComparisonText(f, m, capabilityLabel(f.predictorId)))}</p>`).join("")}<p>${r.validation ? `${e(r.validation.method)}: ${e(r.validation.metric)} ${r.validation.baseline.toFixed(3)} with business context alone; ${r.validation.assessment.toFixed(3)} with assessment added. Lower is better. ${r.validation.folds} folds and ${r.validation.n} people.` : e(r.validationReason)}</p>${r.model.warnings.map((w) => `<p>${e(w)}</p>`).join("")}</section>`;
    })
    .join("");
    ...
    sections.technical
      ? `<section class="appendix"><p class="eyebrow">Supporting evidence</p><h1>Evidence and methods</h1><p>This appendix includes every selected KPI, including inconclusive relationships. Adjusted coefficients describe the relationship with one capability while holding the other selected capabilities and context controls constant.</p><p class="caption">Continuous coefficients use KPI units per score point. Logistic coefficients use log odds. Count coefficients use log rates. Benjamini–Hochberg correction covers all estimable adjusted capability-by-KPI relationships in the run; exploratory correlations form a separate family.</p>${technical}<h2>Inclusion and limitations</h2>${[...payload.quality.warnings, ...payload.result.warnings].map((w) => `<p>${e(w)}</p>`).join("")}${Object.entries(
          ...
          Object.entries(payload.result.libraryVersions)
            .map(([k, v]) => `${k} ${v}`)
            .join(" · "),
        )} · Seed ${payload.result.seed}<br/>Source checksum: ${e(payload.source.checksum)}<br/>Analysis run: ${e(payload.runId)}<br/>Form versions: ${e(payload.source.formVersions.join(", ") || "Not recorded")}</p></section>`
      : ""
  }</body></html>`;
}
