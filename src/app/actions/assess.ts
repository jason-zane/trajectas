...
import {
  ...
  getParticipantReportSnapshotSchema,
...
async function validateAccessTokenImpl(
  ...
  const parsed = validateAccessTokenSchema.safeParse({ token })
  if (!parsed.success) {
...
export async function updateSessionProgress(
  ...
  const parsed = updateSessionProgressSchema.safeParse({ token, sessionId, update })
  if (!parsed.success) {
    return { error: 'Invalid input' }
  }
...
export async function updateSessionProgressLite(
  ...
  const parsed = updateSessionProgressLiteSchema.safeParse({ token, sessionId, update })
  if (!parsed.success) {
    return { error: 'Invalid input' }
  }
...
export async function getParticipantReportSnapshot(
  ...
  const parsed = getParticipantReportSnapshotSchema.safeParse({ token, snapshotId })
