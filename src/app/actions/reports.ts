...
// ---------------------------------------------------------------------------
// Preview data — scoped entities + seeded scores for the report template
// builder. Backed by src/lib/sample-data/seed-preview.ts (which creates rows
// under the Sample Data client whenever an assessment is saved).
// ---------------------------------------------------------------------------
...
/**
 * Returns the entity list (dimensions + factors/constructs) that a given
 * assessment scopes, with each entity carrying a pompScore from the seeded
 * session. Drives the builder's Preview dropdown. If no seed exists yet
 * (brand-new assessment, or the hook failed), falls back to synthScore so
 * the UI never breaks.
 */
export async function getPreviewEntitiesForAssessment(
  ...
  const { data: assessment, error: aErr } = await db
    ...
    .is('deleted_at', null)
    .maybeSingle()
  if (aErr) throwActionError('getPreviewEntitiesForAssessment', 'Unable to load assessment.', aErr)
  if (!assessment) {
    throwActionError('getPreviewEntitiesForAssessment', 'Assessment not found.', new Error('missing'))
  }

  const scoringLevel = (assessment as { scoring_level: 'factor' | 'construct' }).scoring_level

  // Seeded score lookup (empty map if no seed).
...
async function loadConstructLevelEntities(
  ...
  // Constructs linked to the assessment, with their library content.
  ...
  // Find the dimensions those constructs link to (via dimension_constructs),
  // and the weights.
  ...
  // Load dimension library content.
  ...
  const constructEntities: PreviewEntity[] = (constructRows as Array<Record<string, unknown>>).map((c) => {
    ...
    // Use the FIRST dimension link as parent (a construct may link to multiple).
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
async function loadFactorLevelEntities(
  ...
  // Constructs under those factors.
  ...
  // Dimensions those factors sit under.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  // Dimension POMP = mean of its child factors' POMPs.
...
/**
 * Admin-only: seeds every active assessment that doesn't have a preview session
 * yet. Used to backfill existing assessments after the feature first ships.
 */
