...
export function TrajectoryWorkspace({
   ...
   * active, narrow to that dimension's children (including the many-to-many
   * additionalParentIds case). Otherwise return everything.
   */
  const capabilityListSeries = useMemo<TrajectorySeries[]>(() => {
    if (!parentFilter) return filteredFactorSeries
    return filteredFactorSeries.filter(
      (s) =>
        s.parentId === parentFilter ||
        (s.additionalParentIds ?? []).includes(parentFilter),
    )
  }, [filteredFactorSeries, parentFilter])
  ...
  const capabilityChartSeries = useMemo<TrajectorySeries[]>(() => {
    if (parentFilter) return capabilityListSeries
  ...
  }, [capabilityListSeries, parentFilter])
        ...
        <>
          ...
          {viewLevel === 'dimension' ? (
            ...
            <CapabilitiesView
              ...
              chartCapped={
                !parentFilter && capabilityListSeries.length > CAPABILITIES_CHART_TOP_N
              }
            />
          )}
