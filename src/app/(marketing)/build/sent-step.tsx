...
export function SentStep({ email, roleTitle, capabilityCount, token, emailSent, resumed = false, onBuildAnother }: { email: string; roleTitle: string; capabilityCount: number; token: string; emailSent: boolean | null; resumed?: boolean; onBuildAnother: () => void }) {
  ...
  return <section className="rb-ready px-enter"><div className="rb-success-mark"><Check size={26} aria-hidden /></div><h1 tabIndex={-1} data-step-heading>{resumed ? "Your assessment is here." : "Your assessment is ready."}</h1><p>{roleTitle}</p><div className="rb-ready-summary"><span>{capabilityCount} capabilities</span><span>{capabilityCount * PUBLIC_BUILDS_ITEMS_PER_FACTOR} questions</span><span>About {estimatePublicBuildMinutes(capabilityCount)} minutes</span></div>
    ...
    <div className="rb-next"><h2>After you finish</h2><p>Your report will be available in the assessment experience and sent to {email}. You can try another role without finishing this assessment. Your existing link will keep working until it expires.</p><p className="rb-help">Up to 10 new role attempts per person in 24 hours. Save your private assessment link before starting again.</p><button type="button" className="px-text-button" onClick={onBuildAnother}>Build another role <ArrowRight size={16} aria-hidden /></button><h2>Thinking about your team?</h2><p>Talk to us about using Trajectas across your organisation or with your clients.</p><a className="px-link" href="/contact">Talk to Trajectas <ArrowRight size={16} aria-hidden /></a></div>
  </section>;
}
