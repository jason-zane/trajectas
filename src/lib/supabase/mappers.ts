...
export function mapItemRow(row: any): Item {
  return {
    ...
    stem: row.stem,
    stemObserver: row.stem_observer ?? undefined,
...
export function mapCampaignRow(row: any): Campaign {
  return {
    ...
    confidentialityMode: row.confidentiality_mode ?? 'standard',
    inviterName: row.inviter_name ?? undefined,
    inviterRole: row.inviter_role ?? undefined,
    created_at: row.created_at,
    updated_at: row.updated_at ?? undefined,
    deletedAt: row.deleted_at ?? undefined,
  }
