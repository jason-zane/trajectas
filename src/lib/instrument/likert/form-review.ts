...
export function parsePairReview(raw: string, expectedIds: string[]) {
  const result = z.object({ pairs: z.array(z.object({ id: z.string(), redundant: z.boolean(), reason: z.string().trim().min(8).max(1000) })) }).parse(JSON.parse(raw))
