...
import { ArrowRight, ArrowUpRight, Bookmark, Check, ChevronDown, CircleHelp, Download, GitCompareArrows, Layers, LayoutGrid, LineChart, Moon, Sun, Table2, TrendingUp, UserRound, Users, X } from 'lucide-react'
...
export function TrajectoryStudio(props: StudioProps) {
  if (!props.dataset.demo) return <StudioWorkspace {...props} />
...
function StudioWorkspace({ dataset, initialExperience = 'compare', initialLens, onBrowse }: StudioProps) {
  const Content = dataset.demo ? 'main' : 'div'
  ...
  return <div className={`${styles.studio} ${dataset.demo ? styles.withRail : styles.live}`}>
    ...
    <div className={styles.mainShell}>
      ...
      <Content id={dataset.demo ? "studio-main" : undefined} className={dataset.demo ? styles.main : styles.liveContent}>
        <PageHeader className={dataset.demo ? styles.header : styles.liveHeader} title={title} eyebrow="Insights" description={experience === 'individual' ? 'Follow the dimensions of a person’s development.' : experience === 'unified' ? 'See people side by side, then follow what changes.' : 'See each person clearly. Understand the differences.'}>
          <div className={styles.headerActions}><Button variant="outline" onClick={openSaved}><Bookmark />Saved views</Button><Button onClick={() => setExportOpen(true)} disabled={!totalSessions || !validRange}><Download />Export</Button></div>
        </PageHeader>
        <div className={styles.workspace}>
          ...
          <div className={styles.results}>
            <section className={styles.contextBar} aria-label="Assessment and view">
              <label className={styles.assessmentSelect}><span><Layers size={14} />ASSESSMENT</span><div className={styles.assessmentControl}><span aria-hidden="true">{assessment?.name ?? 'Select people to see assessments'}</span><ChevronDown size={16} aria-hidden="true" /><select aria-label="Assessment" value={settings.assessment} onChange={(e) => update({ assessment: e.target.value })}>{!assessments.length && <option value="">Select people to see assessments</option>}{assessments.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</select></div></label>
            ...
            </> : <><div><span>CAMPAIGN RESULTS</span><strong>{snapshotCaption(result, settings)}</strong><small>{totalSessions} selected result{totalSessions === 1 ? '' : 's'} · one completed attempt per person and assessment</small></div><Button variant="outline" onClick={() => setSourcesOpen(true)} disabled={!selected.length}>Choose results<ArrowRight size={14} /></Button></>}</div>
            ...
            {!selected.length ? <div className={styles.emptyPanel}><EmptyState title="Choose people to begin" description="Select people from the panel, then choose an assessment." />{onBrowse && <Button onClick={onBrowse}>Browse workspace participants<ArrowRight size={16} /></Button>}</div> : <>
    ...
    <Dialog open={helpOpen} onOpenChange={setHelpOpen}><DialogContent className={`${styles.dialog} ${dataset.demo ? styles.demoDialog : ''} sm:max-w-xl max-h-[85dvh] overflow-y-auto`}><DialogHeader><DialogTitle>Scores and references</DialogTitle><DialogDescription>{assessment?.name ?? 'Your assessment'}</DialogDescription></DialogHeader><div className={styles.methodology}>
    ...
    <Dialog open={saveOpen} onOpenChange={setSaveOpen}><DialogContent className={`${styles.dialog} ${dataset.demo ? styles.demoDialog : ''} sm:max-w-lg`}><DialogHeader><DialogTitle>Saved views</DialogTitle><DialogDescription>Save selections and settings on this device. Scores are read again from the available dataset when you reopen a view.</DialogDescription></DialogHeader><label className={styles.field}><span>View name</span><input aria-label="View name" maxLength={100} value={viewName} onChange={(e) => setViewName(e.target.value)} /></label><Button disabled={!viewName.trim()} onClick={saveView}><Bookmark size={15} />Save current view</Button>{saved.length > 0 && <div className={styles.savedViews}>{saved.map((entry, index) => <div key={index}><button onClick={() => { const valid = validateSavedSettings(entry.settings, result, dataset.references); if (!valid) { toast.error('This view is no longer available for the current dataset.'); return }; setViews((previous) => ({ ...previous, [entry.experience]: valid })); switchExperience(entry.experience); setSaveOpen(false); toast.success('Saved view opened') }}><Bookmark size={14} /><span><strong>{entry.name}</strong><small>{entry.settings.people.length} people · {entry.settings.lens === 'time' ? 'Over time' : 'Snapshot'}</small></span><ArrowUpRight size={14} /></button><button aria-label={`Delete saved view ${entry.name}`} onClick={() => deleteView(index)}><X size={15} /></button></div>)}</div>}</DialogContent></Dialog>

    <Dialog open={!!sessionDetail} onOpenChange={(open) => { if (!open) setSessionDetail(null) }}><DialogContent className={`${styles.dialog} ${dataset.demo ? styles.demoDialog : ''} sm:max-w-lg`}><DialogHeader><DialogTitle>{sessionDetail?.person}</DialogTitle><DialogDescription>{sessionDetail?.measure} · {displayDate(sessionDetail?.point.completedAt)}</DialogDescription></DialogHeader>{sessionDetail && <><div className={styles.sessionScore}><strong>{score(sessionDetail.point.value)}</strong><span>{sessionDetail.measure} · score / 100</span></div><dl className={styles.sessionDetails}><div><dt>Assessment</dt><dd>{sessionDetail.point.assessmentName}</dd></div><div><dt>Campaign</dt><dd>{sessionDetail.point.campaignTitle}</dd></div><div><dt>Attempt</dt><dd>{sessionDetail.point.attemptNumber}</dd></div><div><dt>Stored interval</dt><dd>{sessionDetail.point.ciLower !== null && sessionDetail.point.ciUpper !== null ? `${score(sessionDetail.point.ciLower)}–${score(sessionDetail.point.ciUpper)}` : 'Not supplied'}</dd></div></dl>{time && <Button variant="outline" onClick={() => { update({ from: sessionDetail.point.completedAt.slice(0, 10) }); setSessionDetail(null) }}>Start history here<ArrowRight size={15} /></Button>}</>}</DialogContent></Dialog>
