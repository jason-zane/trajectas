...
export default async function PreviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const reportTheme = DEFAULT_REPORT_THEME
  const sampleBlocks = generateSampleData(template.blocks as Record<string, unknown>[], reportTheme)
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--report-page-bg, #fafaf8)' }}>
      ...
      <div className="bg-amber-50 border-b border-amber-200 px-4 py-2 text-center text-sm text-amber-800 dark:bg-amber-950/50 dark:border-amber-800 dark:text-amber-200">
        ...
        <a href={`/settings/reports/${id}/builder`} className="underline hover:no-underline">
          Back to builder
        </a>
      </div>
      <Suspense>
