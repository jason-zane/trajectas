  ...
  try {
    ...
    if (state.phase === 'blueprint') {
      ...
      if (!construct) { state.phase = 'generate'; state.specHash = specHash; state.detail = 'Operational facets saved. Generating contrasting wording and scoring directions.' }
        ...
        const planned = checked.facets.map((facet, index) => ({
          id: randomUUID(), facetLabel: facet.label, facetDefinition: facet.definition, intensity: 'mid',
          targetItemCount: Math.floor(spec.itemsPerConstruct / facetCount) + Number(index < spec.itemsPerConstruct % facetCount), displayOrder: index,
        }))
...
Repair feedback: ${JSON.stringify(feedback)}. Rewrite weak ideas or replace them if needed. Set replacesId only to a provided feedback ID when repairing that item; otherwise null. Return ONLY {"items":[{"stem":"...","reverseScored":false,"rationale":"connection to the facet and key direction","replacesId":null}]}.`, raw => {
      ...
      if (!pending.length) { state.phase = 'diversity'; state.detail = 'Independent item reviews complete. Checking wording overlap.' }
        ...
        const batch = blindReviewBatch(spec, pending)
        output.reviewBatch = { itemIds: batch.itemIds, constructIds: batch.constructIds }
        ...
        const results = await Promise.allSettled(state.models.reviewers.map(async (model, index): Promise<ReviewerResult> => {
          let reviewed = await jsonCall(model, reviewPrompt(batch.spec, batch.items, index), raw => parseBlindReview(raw, batch))
          ...
          if (inconsistent.length && Date.now() < deadlineAt - 12_000) {
            ...
            const retryBatch = blindReviewBatch(spec, retryItems)
            const corrections = (output.reviewCorrections ??= []) as Record<string, unknown>[]
            corrections.push({ model, itemIds: retryBatch.itemIds, constructIds: retryBatch.constructIds })
            try {
              const corrected = await jsonCall(model, reviewPrompt(retryBatch.spec, retryBatch.items, index) + '\nYour earlier review had a scoring key and raw response profile that contradicted each other. Re-read these items and the actual numbered anchors. Correct your own judgment; if the item is ambiguous, record that as an issue rather than forcing agreement.', raw => parseBlindReview(raw, retryBatch))
