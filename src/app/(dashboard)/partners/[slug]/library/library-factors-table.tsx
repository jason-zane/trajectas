...
interface LibraryFactorsTableProps {
  rows: TaxonomyAssignmentRow[];
  partnerId: string;
  isPlatformAdmin: boolean;
}

export function LibraryFactorsTable({
  rows: initialRows,
  partnerId,
  isPlatformAdmin,
}: LibraryFactorsTableProps) {
  ...
  function handleToggle(entityId: string, currentlyAssigned: boolean) {
    ...
    startTransition(async () => {
      ...
      toast.success(newState ? "Factor enabled" : "Factor disabled");
    });
  }
  ...
  function handleBulkToggle(assigned: boolean) {
    ...
    startTransition(async () => {
      ...
      toast.success(
        `${ids.length} factor${ids.length !== 1 ? "s" : ""} ${assigned ? "enabled" : "disabled"}`,
      );
      setSelected(new Set());
    });
  }
