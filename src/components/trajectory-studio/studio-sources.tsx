...
export function StudioSources({ dataset, settings, open, onOpenChange, onChange }: {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className={`${styles.dialog} ${dataset.demo ? styles.demoDialog : ''} sm:max-w-2xl max-h-[85dvh] overflow-y-auto`}><DialogHeader><DialogTitle>Choose the results to compare</DialogTitle><DialogDescription>One completed result per person, from a campaign. These selections stay fixed when you explore their history.</DialogDescription></DialogHeader>
    ...
    <Button onClick={() => onOpenChange(false)}>Done<Check size={15} /></Button>
