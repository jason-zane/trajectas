...
export async function CampaignComparePageComponent({
  ...
  // Parameterized header: client surface shows saved comparison metadata if present
  // (this was missing in the admin surface before; now both render it when available)
  const headerEyebrow = saved
    ? (surface === "client" ? `Insights · ${saved.shareScope === 'team' ? 'Shared' : 'Private'}` : 'Insights · Campaign')
    : 'Insights · Campaign'
