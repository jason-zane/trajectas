...
> **SECOND defect, found later (issue #346): this worked example's grid also contains a genuine duplicate.** (1,3), (2,2) and (3,1) are all `diamond, tick 180°` — the 90° tick step aliases every 4 cells on a 3×3 grid (90°×4 = 360°). The JSON above is left as written because its job here is to demonstrate the **storage format** (the two-table split, canonical ordering, provenance sidecar), and the secure-sidecar rationale and Appendix A audit text below are all keyed to these exact numbers. The corrected, duplicate-free grid — 45° tick steps instead of 90°, same key — is in doc 03-logical-reasoning-design.md §6 M6's own "Correction" note, and is what `src/lib/cognitive/generator/families/lrm-2r-xlayer.ts` and `tests/fixtures/cognitive/m6.ts` actually implement. Do not copy the rotation values above into new code; copy them from that family or fixture instead.
...
> **`KEY_EQUALS_CELL` and the "never pick a repeat" heuristic.** Forbidding full-cell coincidence between key and context is necessary: with an RP distractor in every item, a key that *also* copies a cell makes the two indistinguishable on the RP reading. But the forbid has a cost — a test-wise candidate who learns "eliminate any option that reproduces a grid cell" removes one distractor for free, raising the blind-guess rate from .200 to .250.
>
> **CORRECTED, 2026-08-14 — the cost was far larger than one distractor, and this paragraph's own mitigation is not one.** As shipped, five of the eleven families built EVERY distractor as a verbatim copy of a visible cell, and the key was never a copy in any family, so the heuristic did not raise the guess rate from .200 to .250 — it solved the item outright (measured over 12 seeds: LRM-3R-XLAYER 93/93 items, LRM-XOR-DIST-XLAYER 83/83, LRM-2R-XLAYER 95/95, LRM-XOR-XLAYER 73/73, LRM-DIST3X2 78/78, all with 4 of 4 distractors copied). The two worst-affected families are the entire very-hard band, so the bank's hardest items were its easiest to shortcut.
>
> Partial-layer coincidence is NOT an adequate mitigation, because the heuristic compares whole options against whole cells: a key that shares its outer layer with R1C2 still fails to match any cell exactly, and so is still the unique survivor. The real invariant is a partition property — under "is this option a verbatim copy of a visible cell?", the class the key falls into must contain at least two options. Gate **G-11** now enforces exactly that (`qa/degeneracy.ts`'s `copyEliminationCheck`), for every family, in both directions (key-a-copy and key-not-a-copy). The "shares a complete layer" figure is still computed and reported per item as a descriptive statistic; it is no longer mistaken for the gate.
...
- **The blueprint must be authored to hit the bands.** If no family lands in `very_hard`, the bank has no very-hard items, and no amount of generation fixes it. Blueprint coverage is a §9.1 planning problem. (This is exactly what happened at first — see the OQ-1 resolution immediately below and §9.1's revised table.)
- **OQ-1, RESOLVED (issue #346).** doc 03's stated exemplar b values did not reconcile with its own formula: running §4.4 over the exemplars' declared radicals gave M1 = −1.5 (doc said −2.0, under doc's original "2 rules" reading of M1's double progression — the shipped `LRM-PROG-COUNT` family instead declares it as one `direction: 'both'` rule, which reconciles cleanly at −2.0), M6 = +0.5..+0.8 depending on whether the non-cardinal-rotation bonus applies (doc said +0.7) and M8 = +0.9 (doc said +2.2). **The formula is authoritative — the weights above are unchanged, and the exemplars' stated b values in doc 03-logical-reasoning-design.md §4.4/§6 were corrected to match what this formula actually computes**, not the reverse (see that doc's own "Correction" notes for the full reasoning: briefly, this formula is the thing G-14 mechanically re-derives from an item's own radicals on every run, so a hand-typed figure that disagrees with it breaks the audit trail everything else here depends on, and reweighting it to hit three anchor points would be curve-fitting dressed up as a design prior). The practical consequence doc 03's own §9.1 blueprint table already anticipated: **neither M6 nor M8, honestly computed, reaches `very_hard`** — both land in `hard` (+0.8 and +0.9 respectively). §9.1's table below is revised accordingly, and two new families close the gap through genuinely more rule content, not a reweighting.
...
Every gate is a pure function returning `{ status, detail }`, and the composite report is written to `cognitive_item_specs.qa`. `generateFamily` throws on any `fail`. Gates G-01 to G-14, G-18 and G-19 are per-item; G-16 to G-17 are per-batch.
...
| **G-09** | Option homogeneity, **THIRD COMPONENT ADDED 2026-08-14** | No two options structurally identical; element-count spread ≤ 2; no option is a blank or degenerate cell; **and the key is not the lone bulk extremum** — if no other option shares the key's element count, the key must not be the maximum or the minimum. The third clause was added after LRM-ADD was measured: built to doc 03-logical-reasoning-design.md §6 M4's disjoint-operand layout, its key was the union of four disjoint bars and therefore the strict busiest option in **141 of 141 items over 20 seeds**, so "pick the fullest tile" solved the family with no rule extraction at all. `keyBulkExtremumCheck`. | §9 rule 3 | `qa/degeneracy.ts`, `qa/density.ts` |
...
| **G-11** | Copy-elimination resistance, **CORRECTED 2026-08-14** | Partition the 5 options by "is this a verbatim copy of a visible cell?"; the class containing the key must hold ≥ 2 options, so "eliminate every copy" (or its mirror, "pick the copy") can never isolate the key. Subsumes OQ-3's own mitigation, so the `permitKeyEqualsCell` flag no longer branches here. The former wording ("the key shares a complete layer with ≥ 1 context cell") was a *measurement*: it was computed, reported, and then discarded without a threshold for every family that had not set `permitKeyEqualsCell`, which is how the leak shipped. It is still reported in the gate's `detail`. **This gate was necessary but not sufficient — see G-19**: two families satisfied it and remained solvable with certainty by a candidate who chained a second rule-blind cue behind the copy cue. | §3.5 note | `qa/degeneracy.ts` |
| **G-18** | Rule-subset sufficiency (**NEW, 2026-08-14 — beyond doc's original G-01..G-17**) | For a multi-rule item, no single declared rule may leave fewer than 2 options carrying the key's value on its axis. A rule that identifies the key alone makes every other declared rule (and the difficulty credited to it) decorative. Not applicable to single-rule families. The dual check one level up — "no PAIR of rules suffices in a 3-rule item" — is provably incompatible with G-08 at 5 options; see `singleRuleSufficiencyCheck`'s structural-limit note. | new | `qa/degeneracy.ts` |
| **G-19** | Elimination resistance (**NEW, 2026-08-14 — the strengthening of G-11**) | Partition the 5 options by the CONJUNCTION of both rule-blind cues, and require the key's class to hold ≥ 2 options. Cue 1 is G-11's copy question, but read on the **declared rule axes** rather than on full cell identity — otherwise a distractor that copies a visible cell and changes only its fill counts as "novel" to `cellEq` while a candidate discards it on sight (G-08/G-10 read only declared axes, so neither can see the difference either). Cue 2 is "does this option show a count, or a piece of ink, that appears nowhere on the grid?", with features that ARE a declared axis's own value excluded — for those, the cue and the rule are the same act. **Why it exists:** with G-11 alone, chaining the two cues isolated the key in LRM-XOR-DIST-XLAYER **129/129** and LRM-XOR-XLAYER **121/121** items over 20 seeds, versus 0% for the other eight families. | new | `qa/degeneracy.ts` |
...
| **G-17** | Context-blind (batch), **RECONCILED with G-08 (issue #346, item 3)** | The batch-level blind-scorer hit COUNT is exactly 0, confirming G-08's per-item guarantee held for every accepted item in the run (doc's original "hit rate within the exact binomial 95% interval around 0.200" is unreachable once G-08 rejects every item where either scorer recovers the key — see `qa/batch.ts`'s header comment for the full derivation. The theoretical chance-interval figure is still computed and reported for calibration context, but is no longer the pass/fail criterion) | §9 rules 2–4 | `qa/batch.ts` |
...
**Revised (issue #346): the very-hard cell was empty, not merely narrow.** This table originally assumed M8 filled one very-hard pilot slot on its own. Once OQ-1 was resolved (formula authoritative — §3.7 above), M8 recomputes to `hard` (+0.9), and M6, the only other exemplar with a cross-layer/multi-rule profile, also recomputes to `hard` (+0.8). **Doc 03's eight exemplars therefore cover zero of the three very-hard pilot slots.** Two of the ten "new authoring work" families §10 already anticipated were built to close exactly this gap: `LRM-XOR-DIST-XLAYER` (R6+R7, cross-layer; b = +1.8) and `LRM-3R-XLAYER` (R6+R6+R2, cross-layer, three rules; b = +2.20 — corrected 2026-08-14 from +2.35 when `nearMissCount` was reduced 3 → 2, because a three-rule item cannot carry three single-axis near-misses and still clear G-08) — see `src/lib/cognitive/generator/families/lrm-xor-dist-xlayer.ts` and `lrm-3r-xlayer.ts` for the full construction and duplicate-safety proofs. One very-hard pilot slot remains open for a future family.
...
|---|---|---|
| **OQ-1** | **RESOLVED (issue #346).** doc 03's stated exemplar b values do not reconcile with its own §4.4 formula (M1 −2.0 vs −1.5, M6 +0.7 vs +0.5, M8 +2.2 vs +0.6). Gate G-14 fails every exemplar as written | **Decision taken: the formula is authoritative, the stated values were wrong and are corrected — not the reverse.** No weight in `difficulty.ts` was changed (an interaction term or a raised w(R7) would have "fixed" M8 by relabelling it, which was considered and rejected — see doc 03-logical-reasoning-design.md's §4.4 "Correction" note for the full reasoning). M1 reconciles once treated as a single `direction: 'both'` rule (its shipped family does this); M6 and M8's stated values are corrected to +0.8 and +0.9 (both `hard`, not the `very_hard` doc originally claimed for M8) — part of the M6/M8 gap traced to a genuine duplicate cell in each exemplar's original table (part of OQ-2, below), which would have made each item easier than its asserted band regardless of the formula question. Net effect: doc's own eight exemplars cover no very-hard cell; two new families (`LRM-XOR-DIST-XLAYER`, `LRM-3R-XLAYER`) were authored to fill it, per §9.1's revised table |
| **OQ-2** | Six of eight exemplars fail the context-blind gate (Appendix A). **Partially resolved (issue #346):** M6's corrected (duplicate-free) table now passes G-08 via the generator's own repair search — see that item's "Correction" note in doc 03-logical-reasoning-design.md §6 and `tests/fixtures/cognitive/m6.ts`. M1, M2, M3, M5, M7 are unchanged and still fail as documented; M4 and M8 still pass/fail as Appendix A originally found | Repair the remaining exemplars per Appendix A and re-issue doc 03's §6 option sets. The exemplars are the reviewer's calibration set and the fixtures the battery pins against; shipping them with a known leak would train reviewers on the wrong standard |
| **OQ-3** | **RESOLVED (2026-08-14), and the framing was too narrow.** `KEY_EQUALS_CELL` forbids the key duplicating a context cell, which was described as making "eliminate any option that copies a cell" a *free elimination* | **Decision taken: the recommended "≥ 2 cell-copying options" mitigation is right, but it is not a special case for the minority of items where the key coincides — it is the general rule, and it was only ever enforced for `LRM-MOVE`.** G-11 now states it as a partition invariant over all items: whichever class the key falls into under "is this a verbatim copy?", that class must hold ≥ 2 options. Measured cost before the fix was not .200 → .250 but .200 → 1.000 in five of eleven families. Enforcing it required real distractor work, not just a gate: four families now build near-misses by applying a rule *wrongly* to yield a figure that appears nowhere on the grid (a (shape, rotation) or (shape, fill, rotation) recombination the grid does not realise). **FOLLOW-UP CORRECTION (2026-08-14, second pass).** Two of the four — `LRM-XOR-XLAYER` and `LRM-XOR-DIST-XLAYER` — were repaired with *a bar-set of one or three where every cell shows two*, and that repair was wrong for exactly the reason this same entry gives for unregistering `LRM-DIST3X2`. Those two families were in the identical pigeonhole (3 shapes x 3 two-bar sets = 9 distinguishable cells, all 9 consumed by the duplicate-free grid plus the key), so the only non-copy reachable was an out-of-vocabulary import — and importing one trades a one-member class for another one-member class rather than closing anything. Measured over 20 seeds: eliminate the copies, then eliminate the option whose bar count no cell shows, and the key was isolated in **129/129** and **121/121** items. The honest repair is the one this entry already prescribes for `LRM-DIST3X2` — widen the value space so the grid cannot exhaust it. Both families now draw their two-bar sets from all four schema bar positions (C(4,2)=6 sets x 3 shapes = 18 forms against 9 grid positions), so genuine in-vocabulary non-copies exist; see `families/xor-bars.ts` for the construction and its proofs, and gate **G-19** for the invariant that now enforces it. `LRM-DIST3X2` itself remains unregistered pending a redesign onto an axis the grid cannot exhaust (proof in `families/lrm-dist3x2.ts`, restated in its correct and stronger form: while the grid exhausts the vocabulary, *every non-key non-copy must carry a feature value appearing in zero visible cells*) |
...
> **M6's row is historical (issue #346).** The audit above analyses doc 03's ORIGINAL M6 table (90° tick step, which also contained a genuine duplicate cell — see doc 03-logical-reasoning-design.md §6 M6's "Correction" note). The corrected table (45° step, duplicate-free) has different option values and, with the generator's own repair search, **passes** G-08 — see that same "Correction" note and `tests/unit/cognitive-generator-contextblind.test.ts`'s "CORRECTED" test. The M6 row is left as originally computed here because it is what the method above actually measured at the time; it should not be read as still describing the current M6.
