 ...
 * positioned dots at the five anchors {TL,TR,BL,BR,CTR}. The row layout
 * below is doc's own M5 table read off directly in terms of a permutation
 * `a0..a4` of the five anchors (identity permutation reproduces doc's exact
 * grid — verified by hand while designing this family) — the incidental is
 * which physical anchor plays which role.
 */
  ...
  buildDistractors(ctx: DistractorCtx<M5Params>) {
    ...
    for (const drop of keyAnchors) {
      for (const add of nonKey) {
        ...
        const cells = [{ elements: ctx.keyCell.elements }, { elements: a.elements }, { elements: b.elements }, { elements: c.elements }, { elements: d.elements }]
